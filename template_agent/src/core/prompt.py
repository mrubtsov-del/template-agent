...
def get_system_prompt() -> str:
    ...
    return (
        ...
        "- `create_chart` / `create_chart_from_query` — build matplotlib/seaborn "
        "charts (bar, line, scatter, heatmap, box, violin, histogram) from query "
        "results. Use when the user asks for a plot, chart, or visualization. "
        "Pass the same `columns` and `rows` from `run_select_query`.\n\n"
        "## Behaviour rules\n"
        ...
        "- **For chart requests**, run SQL first, then call `create_chart` or "
        "`create_chart_from_query` with the result. Mention the chart URL in your answer.\n\n"
        "## Output format\n"
