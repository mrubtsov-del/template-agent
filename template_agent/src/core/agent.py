...
from template_agent.src.core.tools.google_tools import GOOGLE_TOOLS
...
async def get_template_agent(
    ...
    try:
        ...
        async def connect_with_timeout():
            ...
            server_config = {
                ...
                if sso_token
                else {"apikey": settings.MCP_API_KEY}
            ...
            if not settings.MCP_SSL_VERIFY:
                ...
                servers = {settings.MCP_SERVER_NAME: server_config}

                if settings.ATLAN_MCP_URL:
                    servers["atlan"] = {
                        "url": settings.ATLAN_MCP_URL,
                        "transport": "streamable_http",
                        "headers": {"apikey": settings.ATLAN_MCP_API_KEY}
                        if settings.ATLAN_MCP_API_KEY
                        else {},
                    }
                    logger.info(f"Atlan MCP server added: url={settings.ATLAN_MCP_URL}")
    ...
    if settings.GOOGLE_TOOLS_ENABLED and settings.GOOGLE_APPLICATION_CREDENTIALS_CONTENT:
        tools.extend(GOOGLE_TOOLS)
        logger.info("Loaded %s Google Workspace tools (Sheets/Docs)", len(GOOGLE_TOOLS))
    elif settings.GOOGLE_TOOLS_ENABLED:
        logger.warning(
            "GOOGLE_TOOLS_ENABLED=true but GOOGLE_APPLICATION_CREDENTIALS_CONTENT is not set; "
            "Google Sheets/Docs tools will not be available"
        )
